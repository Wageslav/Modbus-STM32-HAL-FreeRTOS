...
uint8_t RingGetNBytes(modbusRingBuffer_t *xRingBuffer, uint8_t *buffer,
  ...
  if (uNumber > MAX_BUFFER)
    return 0;
  for (uCounter = 0; uCounter < uNumber && uCounter < xRingBuffer->u8available;
       uCounter++) {
  ...
  if (numberHandlers < MAX_M_HANDLERS) {
    ...
    if (modH->uModbusType == MB_SLAVE) {
      ...
      if (modH->xTypeHW == TCP_HW)
        modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave, modH,
                                                &myTaskModbusA_attributesTCP);
      else
#endif
        modH->myTaskModbusAHandle =
            osThreadNew(StartTaskModbusSlave, modH, &myTaskModbusA_attributes);
    } else if (modH->uModbusType == MB_MASTER) {
#if ENABLE_TCP == 1
      if (modH->xTypeHW == TCP_HW)
        modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusMaster, modH,
                                                &myTaskModbusB_attributesTCP);
      else
#endif
        modH->myTaskModbusAHandle =
            osThreadNew(StartTaskModbusMaster, modH, &myTaskModbusB_attributes);

      modH->xTimerTimeout =
          xTimerCreate("xTimerTimeout", modH->u16timeOut, pdFALSE,
    ...
    modH->xTimerT35 =
        xTimerCreate("TimerT35", T35, pdFALSE, (void *)modH->xTimerT35,
...
static mb_err_op_t TCPconnectserver(modbusHandler_t *modH, modbus_t *telegram) {
  ...
  if (clientconn->conn == NULL) {
    ...
    if (clientconn->conn == NULL)
      while (1)
        ;
    err = netconn_connect(clientconn->conn, (ip_addr_t *)&telegram->xIpAddress,
                          telegram->u16Port);
    if (err != ERR_OK) {
      ModbusCloseConnNull(modH);
      return ERR_TIME_OUT;
...
void StartTaskModbusMaster(void *argument) {
  ...
  for (;;) {
    ...
    if (ulNotificationValue) {
      ...
      modH->u16errCnt++;
      xTaskNotify((TaskHandle_t)telegram.u32CurrentTask, modH->i8lastError,
                  eSetValueWithOverwrite);
      continue;
    }
    ...
    if (modH->i8lastError == 0) {
      osSemaphoreRelease(modH->ModBusSphrHandle);
      xTaskNotify((TaskHandle_t)telegram.u32CurrentTask, OP_OK_QUERY,
                  eSetValueWithOverwrite);
    }
  }
}
...
void get_FC1(modbusHandler_t *modH) {
  ...
  for (uint8_t i = 0; i < modH->u8Buffer[2]; i++) {
    if (i % 2)
      modH->u16regs[i / 2] =
          word(modH->u8Buffer[i + u8byte], lowByte(modH->u16regs[i / 2]));
    else
      modH->u16regs[i / 2] =
          word(highByte(modH->u16regs[i / 2]), modH->u8Buffer[i + u8byte]);
  }
}
...
int8_t process_FC1(modbusHandler_t *modH) {
  ...
  uint8_t u8bitsno = 0;
  for (uint16_t u16currentCoil = 0; u16currentCoil < u16Coilno;
       u16currentCoil++) {
