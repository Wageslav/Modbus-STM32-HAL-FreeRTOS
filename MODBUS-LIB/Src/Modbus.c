 ...
 *
 * Full implementation including TCP support and extensions.
 */
...
void ModbusInit(modbusHandler_t *modH)
{
    if (numberHandlers < MAX_M_HANDLERS)
    {
        ...
        else if (modH->uModbusType == MB_MASTER)
        {
            ...
            if (modH->xTimerTimeout == NULL) while(1);

            modH->QueueTelegramHandle = osMessageQueueNew(MAX_TELEGRAMS, sizeof(modbus_t), &QueueTelegram_attributes);
            if (modH->QueueTelegramHandle == NULL) while(1);
        }
        else while(1);

        if (modH->myTaskModbusAHandle == NULL) while(1);

        modH->xTimerT35 = xTimerCreate("TimerT35", T35, pdFALSE,
                                        (void *)modH->xTimerT35,
                                        (TimerCallbackFunction_t)vTimerCallbackT35);
        if (modH->xTimerT35 == NULL) while(1);

        modH->ModBusSphrHandle = osSemaphoreNew(1, 1, &ModBusSphr_attributes);
        if (modH->ModBusSphrHandle == NULL) while(1);
...
void ModbusStart(modbusHandler_t *modH)
{
    ...
    if (modH->xTypeHW == USART_HW_DMA && ENABLE_USART_DMA == 0) while(1);
    ...
    if (modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA)
    {
        ...
        if (modH->uModbusType == MB_SLAVE && modH->u16regs == NULL) while(1);
...
#endif
        {
            if (ModbusPort_UartReceive_IT(modH->port, &modH->dataRX, 1) != MODBUS_UART_OK) while(1);
        }

        if (modH->u8id != 0 && modH->uModbusType == MB_MASTER) while(1);
        if (modH->u8id == 0 && modH->uModbusType == MB_SLAVE) while(1);
    }
...
static mb_err_op_t TCPconnectserver(modbusHandler_t *modH, modbus_t *telegram)
{
    ...
    if (telegram->u8clientID >= NUMBERTCPCONN) return ERR_BAD_TCP_ID;
    clientconn = &modH->newconns[telegram->u8clientID];

    if (clientconn->conn == NULL)
    {
        clientconn->conn = netconn_new(NETCONN_TCP);
        if (clientconn->conn == NULL) while(1);
...
void StartTaskModbusSlave(void *argument)
{
...
#if ENABLE_TCP == 1
    if (modH->xTypeHW == TCP_HW) TCPinitserver(modH);
#endif
    ...
    for (;;)
    {
        ...
        if (modH->xTypeHW == TCP_HW)
        {
            if (TCPwaitConnData(modH) == false) continue;
        }
#endif
        ...
        if (modH->u8Buffer[ID] == ADDRESS_BROADCAST) modH->u8AddressMode = ADDRESS_BROADCAST;
        ...
        if (modH->u8Buffer[ID] != modH->u8id && modH->u8AddressMode != ADDRESS_BROADCAST)
        {
            ...
            continue;
#else
            if (modH->xTypeHW != TCP_HW) continue;
#endif
        }
        ...
        switch (modH->u8Buffer[FUNC])
        {
            ...
            case MB_FC_READ_DISCRETE_INPUT:
                if (modH->u8AddressMode == ADDRESS_BROADCAST) break;
    ...
    for (;;)
    {
        ...
        if (modH->xTypeHW == TCP_HW)
        {
            ...
            if (ulNotificationValue == ERR_OK)
            {
                ...
                if (ulNotificationValue != ERR_OK) ModbusCloseConnNull(modH);
            }
            else ModbusCloseConnNull(modH);
        }
        else
#endif
        {
...
uint8_t validateAnswer(modbusHandler_t *modH)
{
    ...
    {
#endif
        uint16_t u16MsgCRC = (modH->u8Buffer[modH->u8BufferSize-2] << 8) | modH->u8Buffer[modH->u8BufferSize-1];
...
{
    int16_t i16result;
    if (modH->xTypeHW == USART_HW) ModbusPort_UartAbortReceive_IT(modH->port);
    ...
    if (modH->xTypeHW == USART_HW) ModbusPort_UartReceive_IT(modH->port, &modH->dataRX, 1);
    return i16result;
}
...
static void sendTxBuffer(modbusHandler_t *modH)
{
    ...
    if (modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA)
    {
        ...
        if (modH->uModbusType == MB_MASTER) xTimerReset(modH->xTimerTimeout, 0);
#if ENABLE_USB_CDC == 1 || ENABLE_TCP == 1
    }
#if ENABLE_USB_CDC == 1
    ...
    else if (modH->xTypeHW == TCP_HW)
    {
        ...
        if (err != ERR_OK) ModbusCloseConnNull(modH);
...
static int8_t SendQuery(modbusHandler_t *modH, modbus_t telegram)
{
    ...
    switch (telegram.u8fct)
    {
        ...
        case MB_FC_WRITE_COIL:
            modH->u8Buffer[NB_HI] = (telegram.u16reg[0] == 0) ? 0x00 : 0xFF;
            modH->u8Buffer[NB_LO] = 0x00;
            modH->u8BufferSize = 6;
            break;
        ...
        case MB_FC_WRITE_MULTIPLE_COILS:
            data_bytes = (telegram.u16CoilsNo + 7) / 8;
            modH->u8Buffer[BYTE_CNT] = data_bytes;
            for (uint16_t i = 0; i < data_bytes; i++)
                modH->u8Buffer[data_index + i] = ((uint8_t*)telegram.u16reg)[i];
            modH->u8BufferSize = data_index + data_bytes;
            break;

        case MB_FC_WRITE_MULTIPLE_REGISTERS:
            data_bytes = telegram.u16CoilsNo * 2;
            modH->u8Buffer[BYTE_CNT] = data_bytes;
            for (uint16_t i = 0; i < telegram.u16CoilsNo; i++)
            {
                modH->u8Buffer[data_index + i*2] = highByte(telegram.u16reg[i]);
                modH->u8Buffer[data_index + i*2 + 1] = lowByte(telegram.u16reg[i]);
            }
            modH->u8BufferSize = data_index + data_bytes;
            break;
...
int8_t process_FC1(modbusHandler_t *modH)
{
    ...
    uint8_t u8bytesno = (u16Coilno + 7) / 8;
    modH->u8Buffer[ADD_HI] = u8bytesno;
    modH->u8BufferSize = ADD_LO;
    modH->u8Buffer[modH->u8BufferSize + u8bytesno - 1] = 0;

    uint8_t u8bitsno = 0;
    for (uint16_t u16currentCoil = 0; u16currentCoil < u16Coilno; u16currentCoil++)
    {
        uint16_t u16coil = u16StartCoil + u16currentCoil;
        uint16_t u16currentRegister = u16coil / 16;
        uint8_t u8currentBit = u16coil % 16;
...
int8_t process_FC15(modbusHandler_t *modH)
{
    ...
    uint8_t u8bitsno = 0;
    uint8_t u8frameByte = BYTE_CNT + 1;
    for (uint16_t u16currentCoil = 0; u16currentCoil < u16Coilno; u16currentCoil++)
    {
        uint16_t u16coil = u16StartCoil + u16currentCoil;
        uint16_t u16currentRegister = u16coil / 16;
        ...
        bool bTemp = bitRead(modH->u8Buffer[u8frameByte], u8bitsno);
